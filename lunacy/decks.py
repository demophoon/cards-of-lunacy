#!/usr/bin/env python
# encoding: utf-8

default = {
    "black": [
        "_ ain't nothin' to fuck with.",
        "I live my life according to the teachings of _.",
        "The Himalayas are filled with many perils, such as _.",
        "_ gives a whole new meaning to _.",
        "I love _ in my _.",
        "The internet was invented for _.",
        "_ good to the last drop.",
        "I love the smell of _ in the morning.",
        "The Japanese government spent billions of yen researching _.",
        "_ in the front seat; _ in the back seat; gotta make my mind up, which seat can I take?",
        "I met my current significant other at _, The local experimental college is now offering a course on _ led by _.",
        "_ in the morning, sailors take warning.",
        "I might be a _, but at least I'm no _.",
        "The new hip trend at craft bars is vodka infused with _.",
        "_ is a classic case of _.",
        "I really hate _ at work. So I take care of it with _.",
        "The new Nintendo will be called the _.",
        "_ is a gateway to _.",
        "I sleep like a baby after bathing in a tub full of _.",
        "The new version of Dungeons and Dragons makes _ a playable race.",
        "_ is a timesaving godsend for busy housewives.",
        "I want to go to school to study _.",
        "The next ice cream flavor from Ben & Jerry's.  _ is best enjoyed on the first date.",
        "I was into _ before it was cool.",
        "The next Team Fortress 2 update will include _.",
        "_ is dead to me.",
        "I will not eat them Sam-I-Am. I will not eat _.",
        "The only problem with _ is that there isn't enough _ in it.",
        "_ is nothing _ can't fix.",
        "I work out so that I can look good when I'm _.",
        "The power of love is useless against _.",
        "_ is now outsourced to call centers in India.",
        "I'd rather be _.",
        "The road to success is paved with _.",
        "_ is the name of my new indie black metal emo jazz postdubstep funk band.",
        "I'm having trouble finding a college which will allow me to pursue a degree in _.",
        "The sad truth is, at the edge of the universe, there is nothing but _.",
        "_ is the new black.",
        "I've got the fever and the only cure is more _.",
        "The sound of your life, _ radio!",
        "_ is the root of all evil.",
        "If a pot of gold is on one end of the rainbow, what is on the other?",
        "The theme for our senior prom will be _.",
        "_ isn't about sex, it's about power.",
        "If I don't enjoy _, then the terrorists win.",
        "The world is going to end when _ falls on the Earth and _ will be everywhere.",
        "_ means never having to say you're sorry.",
        "If I weren't so busy with _, I'd be _.",
        "There is a time for peace, a time for war, and a time for _.",
        "_ R Us.",
        "If there’s one thing you could change about me it would be _.",
        "There is nothing worse than seeing _ in the light of day.",
        "_, for the win!",
        "In a pinch, _ can be a suitable substitute for _.",
        "There's no crying in _.",
        "_, fun for the whole family!",
        "In a surprising turnaround, _ has publicly come out as supporting President Obama.",
        "There's no such thing as _.",
        "In an astonishing and groundbreaking decision, the Supreme Court has decided in favor of _.",
        "They found _ in the dumpster behind the abortion clinic.",
        "_. Because, fuck you, that's why.",
        "In L.A. County Jail, word is you can trade 200 cigarettes for _.",
        "This decade will be regarded as the golden age of _.",
        "_. I'd hit that.",
        "In Nintendo's new game, you must fight off wave after wave of _ in order to save _.",
        "This Thanksgiving, I'm most thankful for _.",
        "_. It's everywhere you want to be.",
        "In the grim darkness of the future there is only _.",
        "This year's Pulitzer Prize winning novel was chosen for its brilliant _, as a metaphor for _.",
        "_. The Care Bear Stare",
        "In the new tenth circle of Hell, sinners will be condemned to an eternity of _.",
        "Try walking a mile in my _.",
        "_. The Final Frontier.",
        "Instead of endorsing Jell-O Pudding Pops, Bill Cosby now endorses _.",
        "Two things you need to survive on a deserted island. _ and _",
        "_. The Nicolas Cage Story",
        "Instead of her normal self, Cinderella should have turned into _ at midnight.",
        "Warning: This product contains trace amounts of _.",
        "_. The other white meat.",
        "Instead of Robin, Batman should have had _ as a sidekick.",
        "\"Watson!\" Holmes exclaimed, \"This man was obviously killed by _!\"",
        "Instead of the Jews, Hitler should have worried more about _.",
        "What am I hiding from my parents?",
        "Iron Chef: Today's secret ingredient is _.",
        "What am I lying to my friends about?",
        "9 out of 10 doctors agree, _ twice a day prevents cancer.",
        "It is often argued that our ancestors would have never evolved without the aid of _.",
        "What brought the orgy to a screeching halt?",
        "A fortune teller told me I will live a life filled with _.",
        "It rubs _ on its skin or else it gets _ again.",
        "What did I find in the couch cushions?",
        "A hot new alternative medical practice involves placing _ in _.",
        "It sucks when you look in the fridge, and all you have to eat is _.",
        "What do I fear my parents will find in my sock drawer?",
        "A little-known fact is that Microsoft Windows was originally slated to be the first operating system to include _.",
        "It was more uncomfortable than _ at a _ convention.",
        "What do I like best about my job?",
        "A little-known provision of Obamacare allows _ access to _.",
        "Jesus is _.",
        "What do I think about when I'm having trouble getting off?",
        "According to an old Romanian superstition, _ will happen if you don't _ before going to bed.",
        "Keep your friends close, but your _ closer.",
        "What gave me aids?",
        "After Billy Mays died, he sold God _.",
        "Kitten thinks of nothing but _ all day.",
        "What goes best on toast?",
        "_. All the cool kids are doing it.",
        "Legend of Zelda: The _ of _.",
        "What I wouldn't give for 10 minutes alone with _.",
        "An international tribunal has found _ guilty of _.",
        "Like a good neighbor _ is there.",
        "What in my past led to my current position on gun control laws?",
        "And on his farm he had _, e-i-e-i-o!",
        "Loving you is easy 'cause you're _.",
        "What is in my zombie apocalypse survival bag?",
        "And then little Billy woke up the next morning, he discovered that the tooth fairy had left _ under his pillow.",
        "Michael Bay's new three-hour action epic pits _ against _.",
        "What is the moral of the story?",
        "Apple has revealed they now have a patent for _.",
        "What is the only way I can achieve orgasm?",
        "Ask your doctor if _ is right for you.",
        "Moral at work was boosted when they put _ in the vending machines.",
        "What keeps me up at night?",
        "Back in my day, _ only cost a nickel.",
        "Mr. Owl, how many licks does it take to get to the center of _?",
        "What makes Fred Phelps hard?",
        "Bang _ Marry _ Kill _.",
        "My favorite book is the classic tale of _, written by _.",
        "What makes me tingle all over?",
        "Check your privilege. And _.",
        "My favorite childhood memory is _.",
        "What makes the world go round?",
        "Chili’s special tonight: _ with a side of _.",
        "My favorite TV channel is the one that shows programs about _.",
        "What melts in your mouth, not in your hand?",
        "Choosy moms choose _.",
        "My favourite family recipe is _ and _ with just a dash of _.",
        "What time is it?  _ time!",
        "Cirque de Soleil presents MYSTIQUEE, a night of daring acrobatics, whimsy and _.",
        "My Little Pony: Friendship is _.",
        "What was Abraham Lincoln's deep, dark secret?",
        "Daddy drinks because of _.",
        "My mission in life is to create a videogame about _.",
        "What will be an issue in the next presidential election?",
        "Damn it feels good to be _.",
        "My other car is a _.",
        "What will be our last chance for salvation when our new robot overlords take over?",
        "Danger! _ ahead!",
        "New car sticker craze: Calvin pissing on _.",
        "What will be put in the time capsule to preserve humanity's memory?",
        "Dear Sir or Madam, We regret to inform you that the Office of _ has denied your request for _.",
        "Next on Oprah, teens addicted to _.",
        "What will I bring to the party?",
        "Disney presents _ on ice!",
        "Next, on Martha Stewart's Living, spruce up your _ with _.",
        "What's my superpower?",
        "Disney's newest Pixar movie will be about _.",
        "Next, on the Twilight Zone: a parallel universe in which every instance of _ is replaced by _.",
        "What's the real reason that I've gathered you all hear today?",
        "Every _ should be required to get a tattoo of _.",
        "Nobody puts _ in a corner.",
        "What's the worst that could happen?",
        "For campaign funding reform to really be effective, there needs to be a specific policy about _ contributions.",
        "Nothing beats cuddling with _ in bed.",
        "When a man and a woman love each other very much, suddenly _.",
        "Fred Phelps and his church have just announced that they plan to start picketing the funerals of _ as well as gays.",
        "Nothing feels better than _ on _.",
        "When I get drunk, I feel that I am an expert on _.",
        "Genius is 10% inspiration and 90% _.",
        "Now witness the firepower of this fully armed and operational _!",
        "When I get married, our vows will include words about _.",
        "Give me liberty, or give me _!",
        "On tonight's special report, _. Fact or fiction?",
        "When I saw your mom naked, it reminded me of _.",
        "Go-go Gadget _!",
        "One day a man is bitten by _ and is transformed into _.",
        "When I'm alone and the room is totally dark, I like to secretly _.",
        "Groundbreaking research provides convincing evidence that _ killed the dinosaurs.",
        "Only _ is worse than a heretic.",
        "When life gives you _, make _.",
        "Happiness is _.",
        "Our most powerful weapon on the day of the zombie apocalypse will be _.",
        "When you wake up from a night of heavy drinking, you usually regret _.",
        "Hey Rocky, watch me pull _ out of my _!",
        "Remember, _ is never funny. Unless it involves _.",
        "Where do babies come from?",
        "Horton could hear it through the bedroom wall...his mother and _.",
        "Roses are red; Violets are _; _ is _, and so are you.",
        "Who could have guessed that the alien invasion would be easily thwarted by _.",
        "Horton is always forcing his _ on other people.",
        "Say no to _.",
        "Who was my childhood hero?",
        "How did this _ get into my _?",
        "Science has discovered that beyond the edges of the universe, there is only _.",
        "Why am I in such a bad mood?",
        "I always call in sick to work when I have a bad case of _.",
        "Scientists have reverse engineered alien technology that unlocks the secrets of _.",
        "Why am I like this?  I always think of _ when I fall asleep.",
        "Sizzler’s special tonight: _ with a side of _.",
        "Why are we wearing tuxedos?",
        "I can drive and _ and _ at the same time.",
        "Something you should regret, but secretly don't.",
        "Why did the _ have to _?",
        "I can handle public displays of affection, but _ is really too much.",
        "Sometimes you feel like _, sometimes you don't.",
        "Why does God pee on us, periodically?",
        "I can't get no _.",
        "Sticks and stones may break my bones, but _ will never hurt me.",
        "Why is mom calling?",
        "I feel the need—the need for _.",
        "Stop staring at my _.",
        "Why was my application to work at McDonald's rejected?",
        "I firmly believe that for the economy to recover, the Department of _ needs to change its policies on _.",
        "The 1930's is often regarded as the golden age of _.",
        "With the Democrats and Republicans in a dead heat, the election was stolen by the _ party.",
        "I for one welcome our new _ overlords!",
        "The best Christmas present my grandma ever gave me was _.",
        "Wonder Twin powers, activate!  Shape of _.  Form of _.",
        "I got a fever and the only cure is more _.",
        "The best part of waking up is _ in your cup.",
        "You can never have too many _ when in East LA.",
        "I have decided to commit to a life of studious contemplation on _.",
        "The best things in life come in _.",
        "You can never have too much _.",
        "I like my women like _.",
        "The Christian Coalition has recently decided that an even worse sin than same-sex marriage is _.",
        "You know God hates you when _.",
        "I like to keep _ as a souvenir of my sexual encounters.",
        "The clown made all the children laugh with his balloon animal in the shape of _.",
        "You’re in good hands with _.",
        "No Enforcer wants the manage the panel on _.",
        "The most controversial game at PAX this year is an 8-bit indie platformer about _.",
        "_: Achievement unlocked.",
        "In the new DLC for Mass Effect, Shepard must save the galaxy from _.",
        "What made Spock cry?",
        "There was a riot at the Gearbox panel when they gave the attendees _.",
        "What's the latest bullshit that's troubling this quaint fantasy town?",
        "I have an idea even better than Kickstarter, and it's called _ starter.",
        "In the final round of this year's Omegathon, Omeganauts must face off in a game of _.",
        "I don't know exactly how I got the PAX plague, but I suspect it had something to do with _.",
        "You have been waylaid by _ and must defend yourself.",
        "Action stations! Action stations! Set condition one throughout the fleet and brace for _!",
        "Press [down arrow] [down arrow] [left arrow] [right arrow] [B] to unleash _.",
        "In the seventh circle of Hell, sinners must endure _ for all eternity.",
        "Call the law offices of Goldstein & Goldstein, because no one should have to tolerate _ in the workplace.",
        "After months of practice with _, I think I’m finally ready for _.",
        "A successful job interview begins with a firm handshake and ends with _.",
        "When you get right down to it, _ is just _.",
        "To prepare for his upcoming role, Daniel Day-Lewis immersed himself in the world of _.",
        "Lovin’ you is easy ’cause you’re _.",
        "Turns out that _-Man was neither the hero we needed nor wanted.",
        "Finally! A service that delivers _ right to your door.",
        "My life is ruled by a vicious cycle of _ and _.",
        "As part of his daily regimen, Anderson Cooper sets aside 15 minutes for _.",
        "My gym teacher got fired for adding _ to the obstacle course.",
        "The blind date was going horribly until we discovered our shared interest in _.",
        "Money can’t buy me love, but it can buy me _.",
        "Having problems with _? Try _!",
        "_.  Awesome in theory, kind of a mess in practice.",
        "With enough time and pressure, _ will turn into _.",
        "As part of his contract, Prince won’t perform without _ in his dressing room.",
        "I’m not like the rest of you. I’m too rich and busy for _.",
        "And what did you bring for show and tell?",
        "Listen, son. If you want to get involved with _, I won’t stop you. Just steer clear of _.",
        "_: Hours of fun. Easy to use. Perfect for _!",
        "During high school I never really fit in until I found _ club.",
        "What left this stain on my couch?",
        "Hey baby, come back to my place and I’ll show you _.",
        "During his midlife crisis, my dad got really into _.",
        "Next time on Dr. Phil: How to talk to your child about _.",
        "I went from _ to _, all thanks to _.",
        "_ would be woefully incomplete without _.",
        "Only two things in life are certain: death and _.",
        "Members of New York's social elite are paying thousands of dollars just to experience _.",
        "My new favorite porn star is Joey \"_\" McGee.",
        "Everyone down on the ground! We don't want to hurt anyone. We're just here for _.",
        "This month's Cosmo: \"Spice up your sex life by bringing _ into the bedroom.\"",
        "Before I run for president, I must destroy all evidence of my involvement with _.",
        "The healing process began when I joined a support group for victims of _.",
        "Little Miss Muffet Sat on a tuffet, Eating her curds and _.",
        "This is your captain speaking.",
        "Fasten your seatbelts and prepare for _.",
        "The votes are in, and the new high school mascot is _.",
        "If God didn't want us to enjoy _,",
        "he wouldn't have given us _.",
        "In his newest and most difficult stunt, David Blaine must escape from _.",
        "Charades was ruined for me forever when my mom had to act out _.",
        "My country, 'tis of thee, sweet land of _.",
        "The Five Stages of Grief: denial, anger, bargaining, _, acceptance.",
        "Before _, all we had was _.",
        "After months of debate, the Occupy Wall Street General Assembly could only agree on \"More _!\"",
        "My mom freaked out when she looked at my browser history and found _.com/_.",
        "Tonight on 20/20: What you don't know about _ could kill you.",
        "I spent my whole life working toward _, only to have it ruined by _.",
        "You haven't truly lived until you've experienced _ and _ at the same time.",
        "After blacking out during New Year's Eve, I was awoken by _.",
        "This holiday season, Tim Allen must overcome his fear of _ to save Christmas.",
        "What keeps me warm during the cold, cold winter?",
        "Every Christmas, my uncle gets drunk and tells the story about _.",
        "On the third day of Christmas, my true love gave to me: three French hens, two turtle doves and _.",
        "Jesus is _.",
        "Wake up, America. Christmas is under attack by secular liberals and their _.",
        "_? There's an app for that",
        "Why can't I sleep at night?",
        "What's that smell?",
        "I got 99 problems but _ ain't one.",
        "Who stole the cookies from the cookie jar?",
        "What's the next Happy Meal (r) toy?",
        "Anthropologists have recently discovered a primitive tribe that worships _.",
        "It's a pity that kids these days are all getting involved with _.",
        "During Picasso's often-overlooked Brown Period, he produced hundreds of paintings of _.",
        "Alternative medicine is now embracing the curative powers of _.",
        "And the Academy Award for _ goes to _.",
        "What's that sound?",
        "What ended my last relationship?",
        "MTV's new reality TV show features eight washed-up celebrities living with _.",
        "I drink to forget _.",
        "I'm sorry, I couldn't complete my homework because of _.",
        "What is Batman's guilty pleasure?",
        "This is the way the world ends This is the way the world ends Not with a bang but with _.",
        "What's a girl's best friend?",
        "TSA guidelines now prohibit _ on airplanes.",
        "_. That's how I want to die.",
        "For my next trick, I will pull _ out of _.",
        "In the new Disney Channel Original Movie, Hannah Montana struggles with _ for the first time.",
        "_ is a slippery slope that leads to _.",
        "What does Dick Cheney prefer?",
        "I wish I hadn't lost the instruction manual for _.",
        "Instead of coal, Santa now gives the bad children _.",
        "What's the most emo?",
        "In 1,000 years, when paper money is but a distant memory, _ will be our currency.",
        "What's the next superhero/sidekick duo? _ _",
        "In M. Night Shyamalan's new movie, Bruce Willis discovers that _ had really been _ all along",
        "A romantic, candlelit dinner would be incomplete without _.",
        "_. Betcha can't have just one!",
        "White people like _.",
        "_. High five, bro.",
        "Next from J.K. Rowling: Harry Potter and Chamber of _.",
        "BILLY MAYS HERE FOR _.",
        "In a world ravaged by _, our only solace is _.",
        "War! What is it good for?",
        "During sex, I like to think about _.",
        "What are my parents hiding from me?",
        "What will always get you laid?",
        "When I'm in prison, I'll have _ smuggled in.",
        "What did I bring back from Mexico?",
        "What don't you want to find in your Chinese food?",
        "What will I bring back in time to convince people that I am a powerful wizard?",
        "How am I maintaining my relationship status?",
        "Coming to Broadway this season, _: The Musical.",
        "While the United States raced the Soviet Union to the moon, the Mexican government funnelled millions of pesos into research on _.",
        "After Hurricane Katrina, Sean Penn brought _ to the people of New Orleans.",
        "Due to a PR fiasco, Walmart no longer offers _.",
        "In his new summer commedy, Rob Schneider is _ trapped in the body of _.",
        "Rumour has it that Vladimir Putin's favorite dish is _ stuffed with _.",
        "But before I kill you Mr. Bond, I must show you _.",
        "What gives me uncontrollable gas?",
        "What do old people smell like?",
        "The class field trip was completely ruined by _.",
        "When Pharaoh remained unmoved, Moses called down a Plague of _.",
        "What's my secret power?",
        "what's there a ton of in heaven?",
        "What would grandma find disturbing, yet oddly charming?",
        "I never truly understood _ until I encountered _.",
        "The US has begun airdropping _ to the children of Afghanistan.",
        "What helps Obama unwind?",
        "What did Vin Diesel eat for dinner?",
        "_: good to the last drop.",
        "Why am I sticky?",
        "What gets better with age?",
        "_: kid-tested, mother-approved.",
        "What's the crustiest?",
        "What's Teach for America using to inspire inner city students to succeed?",
        "Studies show that lab rats navigate mazes 50% faster after being exposed to _.",
        "Life was difficult for cavemen before _.",
        "Make a haiku. _ _ _",
        "I do not know with what weapons World War III will be fought, but World War IV will be fought with _.",
        "Why do I hurt all over?",
        "What am I giving up for Lent?",
        "In Michael Jackson's final moments, he thought about _.",
        "In an attempt to reach a wider audience, the Smithsonian Museum of Natural History has opened an interactive exhibit on _.",
        "When I am President of the United States, I will create the department of _.",
        "Lifetime (r) presents _, the story of _.",
        "When I am a billionaire, I shall erect a 50-foot statue to commemorate _.",
        "When I was tripping on acid, _ turned into _.",
        "That's right, I killed _. How, you ask? _.",
        "What's my anti-drug?",
        "_ + _ = _.",
        "What never fails to liven up the party?",
        "What's the new fad diet?",
        "Major League Baseball has banned _ for giving players an unfair advantage.",
        "If I got three wishes from a genie, I'd wish for _ _ _.",
        "_, gotta catch them all",
        "_: The Story of _",
        "I caught mommy under the mistletoe doing _.",
        "You were doing what?",
        "Last night I _"
    ],
    "white": [
        "Satan's Jock-Strap.",
        "Cooking them alive, because the fear adds flavor.",
        "Bitch, where's my money!?",
        "Bus driver, uppercuts.",
        "Terror Flesh",
        "BILLY MAYS",
        "Ass Blasting",
        "The Shake Weight",
        "That \"Not So Fresh Feeling\"",
        "Shitting your pants to end a bad date",
        "Playing \"The Penis Game\" at funerals",
        "Pikachu",
        "Not getting caught Being \"Only a little pregnant\"",
        "'Murica",
        "NASCAR",
        "Performing sex acts learned from Urbandictionary.com",
        "Motherfucking Cthuhlu",
        "Morning wood",
        "Gluing captions to cats",
        "GHOSTBUSTERS!",
        "Forgetting the safe word",
        "FALCON PUNCH!",
        "Drunk texts",
        "Attempting to stop a chainsaw with your genitals",
        "A Vietnamese shame wheel",
        "A tiny dog humping everything in its path",
        "Alex Trebek",
        "A haunted carousel",
        "A fancy space whore",
        "A cheese-grater to the dick $56 in pennies",
        "A self-fulfilling prophecy",
        "Hardcore Aztec death sports",
        "The ancient art of",
        "Thai kick-fucking",
        "Beekeeping in the nude",
        "Getting down to business to defeat the Huns",
        "Herpes",
        "A toddler-sized coffin",
        "A filthy litter-box",
        "Feline STDs",
        "A truck that just screams \"I HAVE A TINY PENIS!\"",
        "Accidentally sexting your mom",
        "Being the mayor of drunk town",
        "Farting so hard it sets off the smoke detector",
        "Tabasco Sauce in your urethra",
        "My very own motherfucking dragon",
        "Placing 2nd in a dick sucking contest",
        "Literally giving a flying fuck",
        "Sucking your own dick",
        "A slow clap",
        "A perfectly crafted Victorian doll house with working plumbing and electricity",
        "Using foreign objects to pleasure yourself",
        "Liberty and justice for all",
        "Pissing on everything you love",
        "A T-Rex caught in a cyclone helplessly flailing its tiny arms",
        "A solid foot-job",
        "Absolutely Fuck-All",
        "A man sized hamster wheel",
        "Having assholes for eyes",
        "Drunk toddlers",
        "Cum Dumpster",
        "2 Girls 1 Cup",
        "One trillion dollars",
        "Cunt 50 Shades of Grey",
        "Oprah's Va-Jay-Jay",
        "Cutting his dick off",
        "70's bush",
        "Our homeless veterans",
        "David Hasselhoff",
        "A 60 year old who is hot in a weird way",
        "Paris Hilton dead puppies",
        "A Bag of Milky Ways",
        "PCP",
        "Defying the laws of physics",
        "A bar-mitzvahs",
        "Pedophilia",
        "Detroit",
        "A beat so fresh, it just might destroy the universe",
        "Penguin orgy",
        "Diarrhea",
        "A bitch slap",
        "Penis implants",
        "Discovering you're the main dish at a cannibal gathering",
        "A bitchy email from your boss",
        "People who smell their own socks",
        "Disgruntled Apple fanboys",
        "A black hole",
        "Performing sex acts learned from Urban Dictionary",
        "Disney-themed weddings",
        "A black-and-white episode of \"Gilligan's Island\"",
        "Pirate customer service representatives",
        "Donkey Shows",
        "A box full of terrible foreign candy",
        "Pizza, Pot and Porn",
        "Down's Syndrome",
        "A bucket of angry tiger cubs",
        "Pooping in the soup",
        "Dressing up in blackface",
        "A bushy mustache",
        "Popeye's Chicken",
        "Drop-kicking anti-semitic lesbians",
        "A car trunk full of hot, glistening tripe",
        "Prison Rape",
        "Dropping your iPhone into a urinal",
        "A chainmail bikini",
        "Prosti-tots",
        "Drunk-texting an ex",
        "A cheap motel",
        "Public restrooms",
        "East Indians",
        "A cheesy discharge",
        "Puns that aren't funny",
        "Eating the yellow snow",
        "A cripple fight",
        "Putting out on the first date",
        "Editing Wikipedia to win an argument",
        "A crippled albino midget",
        "Putting the \"fun\" in \"fundamentalism\"",
        "Elderly abuse",
        "A crying stripper",
        "Putting wheel locks on wheelchairs",
        "Ellen Degeneres",
        "A day care",
        "Fight Club",
        "Queef Latifah",
        "Encouraging children to eat lead paint chips",
        "A dead hooker",
        "Quoting Monty Python",
        "Encouraging your passenger to moon a bus full of children when you pass it",
        "A dildo",
        "Randy gerbils totally macking you",
        "Etsy",
        "A dude in a thong",
        "Reading Rainbow",
        "Eugenics for fun and profit",
        "A failed abortion",
        "Realdoll owners",
        "Europeans",
        "A fat ass in spandex",
        "Reality television",
        "Euthanasia",
        "A foul mouthed Furby",
        "Realizing too late that there's no toilet paper left",
        "Excruciatingly graphic Sonic the Hedgehog fan fiction",
        "A freshly waxed handle bar mustache",
        "Reparations for slavery",
        "Extremely tight pants",
        "A gas station bathroom",
        "Richard Simmons in a pink sparkly tank-top",
        "Faith healing",
        "A glass jar with a preserved two-headed fetal pig",
        "Riding the short bus",
        "Farting into your petticoats and wafting it at Lord Gregory",
        "A gold digger",
        "Robbing a sperm bank",
        "Fat American children",
        "A grown man crying at a movie",
        "Rock-Paper-Scissors-Lizard-Spock",
        "Fellatio",
        "A Hello Kitty vibrator",
        "Rodent feces",
        "Feminists with facial hair",
        "A hilarious failed suicide attempt",
        "Ron Jeremy's penis finding out that your lover is gay",
        "A horse with a boner",
        "Rotting elk carcasses",
        "Finding your roommate passed out wearing a clown costume in a public bathroom",
        "A husband",
        "Rusty Trombone",
        "Finishing off a pint of Ben & Jerry's and knowing you'd have kept eating more if it had been a gallon container",
        "A man pretending to be a woman's \"gay friend\" in order to get at that sweet vadge",
        "Salmonella",
        "First world problems",
        "A man shot over a game of dominoes",
        "Sand in My Vagina",
        "Fisting",
        "A man-sized hamster wheel",
        "Saying \"I disapprove of what you say, but I will defend to the death your right to say it\" when you know you wouldn't",
        "Fleshlights",
        "A Maury Povitch paternity test",
        "Seabirds strangled by plastic six-pack rings",
        "Forgetting the safe word",
        "A millionaire with a heart of gold",
        "Secondhand adult diapers",
        "Forwarding your grandmother all your erectile dysfunction pill spam",
        "A molester mustache",
        "Self-diagnosed Asperger's Syndrome sufferers",
        "Frantically clicking pictures of cats on the internet",
        "A monolith",
        "Semen",
        "Fred Phelps being sodomized with a Wookie (Yes, With, Not By)",
        "A ninja with diarrhea",
        "Sex with vegetables",
        "Free candy from strangers",
        "A pack of Mentos and a two-liter bottle of Diet Coke",
        "Sexual experimentation in college",
        "Front butt",
        "A pearl necklace",
        "Shaken Baby Syndrome",
        "Fuck You",
        "A person who is a douche but still hot",
        "Sharting",
        "Fucking magnets, how do they work?",
        "A piñata full of scorpions",
        "Sheep",
        "Fucking while watching Schindler's List",
        "A porn star with daddy issues",
        "Showing up to an orgy for the food",
        "Furries Yeah, Furries",
        "A prosthetic leg",
        "Sipping artificial popcorn butter",
        "Gaining 100 pounds after you get married",
        "A racist uncle",
        "Slaughtering innocent children",
        "Gay cowboys eating pudding",
        "A really ugly baby",
        "Slavery",
        "Gay for pay",
        "A rubber snake",
        "Slutty nuns",
        "Gay Midget Donkey Porn",
        "A rusty trombone",
        "Smelly hippies",
        "Gay Twilight Porn",
        "A sadistic dentist",
        "Smelly Vaginas",
        "Gay-marrying your prison rapist",
        "A self-fulfilling prophecy",
        "Smurf-on-smurfette action",
        "Genital warts",
        "A suspiciously convenient plot device",
        "Snitching",
        "Gerbils",
        "A televangelist getting caught picking up male prostitutes",
        "Snookis Vagina Getting \"unfriended\" on Facebook",
        "A time machine",
        "Speaking English instead of American",
        "Getting a lapdance from Hilary Clinton",
        "A toddler-sized coffin",
        "Squirrel-on-whale action",
        "Getting angry at something just to pass the time",
        "A tramp stamp",
        "Starting a cult for tax breaks",
        "Getting unfriended by your mom on Facebook",
        "A vintage collection of Kirk-Spock slashfic",
        "Stealing Babies",
        "Getting Your Period For The First Time",
        "A wet willy",
        "Stealing Wheelchairs",
        "Giant squid",
        "A wheelchair death race",
        "Stepping in gum",
        "Gingers",
        "A whiny Taylor Swift song about a guy screwing her over",
        "Stepping on a hairball with a bare foot",
        "Girl Scouts on steroids",
        "A wife",
        "Stigmata",
        "Girls Gone Wild",
        "A wisecracking terrorist",
        "Stormtroopers",
        "Girls who shouldn't go wild",
        "A wizard's sleeve",
        "Stunt cock",
        "Giving over to the dark side",
        "A woman scorned",
        "Suburban chain restaurants",
        "God Hates You",
        "A woman scorned",
        "Sucking some dicks to not get drafted",
        "Going on a Pop-Tart bender",
        "A yeast infection",
        "Suicidal Foxconn employees",
        "Going to black Friday sales at 4am",
        "Accidentally sexting your mom",
        "Swingers",
        "Hair in the shower drain",
        "Adult diapers",
        "Synchronized Sneezing",
        "Having a PhD in 'experimenting' with homosexuality",
        "Adult friendfinder",
        "Syrup Sex",
        "Having dinner with your ex",
        "Amputee sex",
        "Taking a dump in the ball pit at Chuck E Cheese",
        "Having sex with a bowl of Jello",
        "An attractive midget",
        "Taking body shots off your mom",
        "Helen Keller",
        "An awkward boner",
        "Taking fantasy football way too seriously",
        "Helen Keller’s Driving school",
        "An Explosion of",
        "Juices in My Mouth",
        "Teen Pregnancy",
        "Hello Kitty",
        "An uncomfortably attractive pre-op tranny",
        "Telling a nun to eat a dick",
        "Hello!",
        "Anal Leakage",
        "Texas Sleigh Rides",
        "Herpes",
        "Anal warts",
        "Thai Lady Boys",
        "Hillary Clinton's gynecologist",
        "Anne Frank",
        "That \"not-so-fresh\" feeling",
        "Hitler’s House of Barbecue",
        "Anyone who puts Baby in a corner",
        "That crazy bitch you work with",
        "Hoarders",
        "Asians",
        "That feeling when you encounter a handicapped person waiting for you to leave the largest stall in a restroom",
        "Homoerotic frat boys",
        "Asians being racist against each other",
        "That guy who is pretty much a dick but you're still friends with him anyway",
        "Hugging your grandma while you have an erection",
        "Asking her when she's due and finding out she's not pregnant",
        "That stuff between your toes",
        "Human sacrifice",
        "Ass-to-mouth",
        "The 9/11 Truther movement",
        "Hunting accidents",
        "Assholes",
        "The ancient martial art of Flung Poo",
        "Hymen reconstruction surgery",
        "Assuming all handicapped people are retarded",
        "The Boy Scouts of America",
        "I love you",
        "Atheists and agnostics",
        "The cast of Jersey Shore",
        "I'm friends with your dad on Facebook",
        "Attending church",
        "The Challenger explosion",
        "Illegal immigrants",
        "Autistic Kids",
        "The CIA selling crack in the inner-city",
        "Improvised explosive devices",
        "Auto-erotic asphyxiation",
        "The Civil War",
        "Indecent exposure",
        "Baby pigs wearing people clothes",
        "The Dick in My Mouth",
        "Indian Food Poops",
        "Back hair",
        "The drama club",
        "Infuriating Incompetence",
        "Band geeks",
        "The dreams of a child",
        "Insatiable blood lust",
        "Barely 18 Chaz Bono",
        "The entire Mormon Tabernacle Choir",
        "Insatiable bloodlust",
        "Bath salts",
        "The female orgasm",
        "Ironic facial hair",
        "Bea Arthur’s Saggy Boobs",
        "The harsh day of light",
        "Jackass 19 year olds who wear sunglasses at the poker table",
        "Bears",
        "The Holy Grail",
        "Jacking Off Onto The Wedding Cake And Telling Everyone It's Just Frosting",
        "Beating Rodney King",
        "The Human Centipede",
        "Jacking off to Saturday morning cartoons",
        "Because Fuck You, That's Why",
        "The Jonestown cult suicides",
        "Jailbait",
        "Becoming a Junior High Chess Champion for the sweet hook-ups",
        "The Kardashian sisters",
        "Japanese schoolgirl porn",
        "Becoming what you hate",
        "The Knights Who Say \"Ni\"!",
        "Jerry Sandusky’s Security Blanket",
        "Bedbugs",
        "The loneliest brony",
        "Jerry’s Kids",
        "Beef",
        "The Maury Show",
        "Jesus",
        "Beer Farts",
        "The orchestra that played as the Titanic went down",
        "Jock Itch",
        "Being \"a little pregnant\"",
        "The Red Menace",
        "Joe Biden's recurring wet dream",
        "Being friends with someone who is awful, but frequently picks up the check at dinner",
        "The rich pageantry of Macy's Thanksgiving Day Parade",
        "Kibbles'n Bits",
        "Being happy you just made a gay friend so you can use the phrase \"my gay friend\"...",
        "The Ron Popeil Pocket Fisherman",
        "Kicking a man while he's Tebowing",
        "Being Raped By A Moose",
        "The second time I got crabs",
        "Kicking Puppies",
        "Being turned on by phallic vegetables",
        "The Spanish Inquisition",
        "Kumquats",
        "Being uncomfortable realizing you're outnumbered by members of another race",
        "The Tea Party",
        "Ladies' underwear purchased from a vending machine",
        "Belly Button Lint",
        "The three wise men",
        "Laughing at orphans",
        "Biggest Losers",
        "The two actors who played Darren",
        "Stevens in \"Bewitched\"",
        "Lesbian stripper ninjas",
        "Bin Laden",
        "The wet spot on the bed",
        "Lolcats",
        "Binders full of women",
        "This is a test",
        "Losing your virginity to someone you just met at a frat party",
        "Black Jesus",
        "This is what I'm saying!",
        "Lying to a girl to get her to put out",
        "Blacking out",
        "Three and a half babies",
        "Lying to terminally ill children about their life expectancy",
        "Blood diamonds",
        "Three Asian strippers, two porn stars, and a suitcase full of blow",
        "Lyme Disease",
        "Bloody poop",
        "Throwing a virgin into a volcano",
        "M-m-m-mocking st-st-st-stutterers",
        "Blue Waffle",
        "Throwing Poop like a Monkey",
        "Making a girl pay on the first date",
        "Boobs that sag down to her knees",
        "Throwing up in your mouth",
        "Making love to a really sexy sheep",
        "Botched facelift",
        "Tickle Me Elmo",
        "Male pregnancy",
        "Botulism",
        "Time travel",
        "Man Boobs",
        "Boy bands",
        "Toast with the burned image of Our Lord and Savior, Jesus Christ",
        "Masturbating With Glue",
        "Breaking into a zoo to steal a monkey",
        "Toenail fungus",
        "Me in a thong",
        "Breast Cancer",
        "Tossing Salad",
        "Mecha-Hitler",
        "Breath so bad that your butt is mad at your mouth",
        "Traveling back in time to become your own grandparent",
        "Bunnies",
        "Triangle Bush",
        "Mel Gibson's sanity",
        "Burning in hell",
        "Trying to be nice while describing a friend's ugly baby",
        "Metal Gear",
        "Burning when you pee",
        "Tuna noodle casserole",
        "Michigan",
        "Butt acne",
        "Twincest",
        "Mistaking a retarded person for someone who's merely deaf",
        "Butt Hair",
        "Ugly Mexican hookers",
        "Misunderstood pedophiles",
        "Butter",
        "Unconditional love",
        "Mocking autistic children because they don't have feelings",
        "Butthurt Underage cosplayers in bondage gear",
        "Monkeys Fucking",
        "Canned tuna with extra dolphin",
        "Unidentifiable spots on your dick",
        "Mother Theresa's rotting corpse",
        "Cards Against Humanity",
        "Unironic Tebowing",
        "Carrot Top's pecs",
        "Vagina dentata",
        "Muhammed Ali",
        "Cartoon pussy",
        "Victims of lead-based paint",
        "Muscles",
        "Celebrity beaver shot",
        "Vorpal Dildo of Enlightenment +2",
        "Muslims",
        "Cellophane underwear",
        "War crimes",
        "My dick in a box",
        "Charlie Sheen",
        "Watching a Twilight movie on purpose",
        "My hopes and dreams",
        "Childrens' crutches",
        "Wearing extra thin pants for a strip club lap dance",
        "NCIS Fanfiction",
        "Chimpanzees with anger management issues",
        "Wearing the genitals of a defeated enemy around your neck as a trophy",
        "Never being able to look your dog in the eyes again",
        "Chris Farley seductively rubbing his nipples",
        "White Trash",
        "New England Clam Chowder",
        "Chubby Chasers",
        "Wolf-whistling at Girl Scouts",
        "Nicholas Cage's storied career",
        "Chuck Norris",
        "Women",
        "Not knowing the difference between trolling and being wrong",
        "Chunks of dead prostitute",
        "Wondering if it was something you did which caused your significant other to become gay",
        "Not using enough lube",
        "Classic Mickey",
        "Mouse porn",
        "Wondering if the colorblind have an equally useless superpower",
        "NPR",
        "Clubbing baby seals",
        "World Series of Felching",
        "Nude wrestling",
        "Consensual rape",
        "Yellow snow",
        "Nuns",
        "Cookies and kittens",
        "You are a whore",
        "Cousin cuddling",
        "Your ass hurting from wiping too much",
        "Off-duty Nazis just looking for a good time",
        "Cracking your knuckles",
        "Your aunt barging into the bathroom when you're pooping",
        "Old ladies paying by check",
        "Cripple Fight",
        "Your face",
        "Old Man Penis",
        "Crotchless toddler underwear",
        "Zombie consultants",
        "Old people having loud sex",
        "Cryogenically frozen testicles",
        "Zyklon B",
        "Full HD.",
        "Gamergate.",
        "Mario Kart rage.",
        "Tapping Serra Angel.",
        "The gravity gun.",
        "Nude-modding Super Mario World.",
        "Charles Barkley Shut Up and Jam!",
        "Reading the comments.",
        "An angry stone head that stomps on the floor every three seconds.",
        "Legendary Creature - Robert Khoo.",
        "70,000 gamers sweating and farting inside an airtight steel dome.",
        "Yoshi's huge egg-laying cloaca.",
        "Winning the approval of Cooking Mama that you never got from actual mama.",
        "Allowing nacho cheese to curdle in your beard while you creep in League of Legends.",
        "The Cock Ring of Alacrity.",
        "Eating a pizza that's lying in the street to gain health.",
        "Achieving the manual dexterity and tactical brilliance of a 12- year-old Korean boy.",
        "Offering sexual favors for an ore and a sheep.",
        "Getting into a situation with an owlbear.",
        "Rolling a D20 to save a failing marriage.",
        "A homemade, cum-stained Star Trek uniform.",
        "Grand Theft Auto: Fort Lauderdale.",
        "The collective wail of every Magic: The Gathering player suddenly realizing that they've spent hundreds of dollars on pieces of cardboard.",
        "Unlocking a new sex position.",
        "A madman who lives in a policebox and kidnaps women.",
        "Being an attractive elf trapped in an unattractive human's body.",
        "The boner hatch in the Iron Man suit.",
        "SNES cartridge cleaning fluid.",
        "Temporary invincibility.",
        "Never watching, discussing, or thinking about My Little Pony.",
        "Google Glass + ecigarette: Ultimate Combo!",
        "The Sarlacc.",
        "Turn-of-the-century sky racists.",
        "Buying virtual clothes for a Sim family instead of real clothes for a real family.",
        "Filling every pouch of a UtiliKilt(TM) with pizza.",
        "The decade of legal inquests following a single hour of Grand Theft Auto.",
        "Bowser's aching heart.",
        "A fully-dressed female videogame character.",
        "Paying the iron price.",
        "Loading from a previous save.",
        "The Klobb.",
        "Jiggle physics.",
        "Sharpening a foam broadsword on a foam whetstone.",
        "Wil Wheaton crashing an actual spaceship.",
        "Running out of stamina.",
        "The rocket launcher.",
        "Forgetting to eat, and consequently dying.",
        "An immediately regrettable $9 hot dog from the Boston Convention Center.",
        "The depression that ensues after catching 'em all.",
        "Achieving 500 actions per minute.",
        "Casting magic missile at a bully.",
        "Violating the First Law of Robotics.",
        "Smashing all the pottery in a Pottery Barn in search of rupees.",
        "Getting bitch slapped by Dhalsim.",
        "Getting inside the Horadric Cube with a hot babe and pressing the transmute button.",
        "Judging elves by the color of their skin and not by the content of their character.",
        "Firefly: Season 2.",
        "Punching a tree to gather wood.",
        "Charging up all the way.",
        "Rotating shapes in mid-air so that they fit into other shapes when they fall.",
        "Spending the year's insulin budget on Warhammer 40k figurines.",
        "Vespene gas.",
        "That ass.",
        "Having sex on top of a pizza.",
        "Three months in the hole.",
        "Nothing.",
        "Indescribable loneliness.",
        "A botched circumcision.",
        "Shutting the fuck up.",
        "An ass disaster.",
        "The Land of Chocolate.",
        "The primal, ballslapping sex your parents are having right now.",
        "All my friends dying.",
        "Slapping a racist old lady.",
        "A cat video so cute that your eyes roll back and your spine slides out of your anus.",
        "Putting an entire peanut buter and jelly sandwich into the VCR.",
        "A lamprey swimming up the toilet and latching onto your taint.",
        "Cock.",
        "Spending lots of money.",
        "Jumping out at people.",
        "A cop who is also a dog.",
        "Some douche with an acoustic guitar.",
        "A black male in his early 20s, last seen wearing a hoodie.",
        "Dying alone and in pain.",
        "Flying robots that kill people.",
        "Mufasa’s death scene.",
        "Gay aliens.",
        "A greased-up Matthew McConaughey.",
        "Bill Clinton, naked on a bearskin rug with a saxophone.",
        "The way white people is.",
        "An unstoppable wave of fire ants.",
        "Demonic possession.",
        "Reverse cowgirl.",
        "Not contributing to society in any meaningful way.",
        "The Harlem Globetrotters.",
        "The Quesadilla Explosion Salad from Chili’s.",
        "An all-midget production of Shakespeare’s Richard III.",
        "Vomiting midblowjob.",
        "Actually getting shot, for real.",
        "Screaming like a maniac.",
        "My manservant, Claude.",
        "Not having sex.",
        "The moist, demanding chasm of his mouth.",
        "Having shotguns for legs.",
        "Vietnam flashbacks.",
        "Filling every orifice with butterscotch pudding.",
        "Letting everyone down.",
        "Running naked through a mall, pissing and shitting everywhere.",
        "Unlimited soup, salad, and breadsticks.",
        "A spontaneous conga line.",
        "Warm, velvety muppet sex.",
        "Crying into the pages of Sylvia Plath.",
        "A vagina that leads to another dimension.",
        "Self-flagellation.",
        "Velcro.",
        "Disco fever.",
        "The systematic destruction of an entire people and their way of life.",
        "A PowerPoint presentation.",
        "Getting your dick stuck in a Chinese finger trap with another dick.",
        "Samuel L. Jackson.",
        "A surprising amount of hair.",
        "Fisting.",
        "A boo-boo.",
        "Eating Tom Selleck’s mustache to gain his powers.",
        "The thin veneer of situational causality that underlies porn.",
        "Going around punching people.",
        "Roland the Farter, flatulist to the king.",
        "Girls that always be textin’.",
        "The entire Internet.",
        "A pile of squirming bodies.",
        "Blowing some dudes in an alley.",
        "Some kind of birdman.",
        "Buying the right pants to be cool.",
        "Drinking ten 5-hour ENERGYs to get fifty continuous hours of energy.",
        "Chugging a lava lamp.",
        "Blood farts.",
        "Sneezing, farting, and coming at the same time.",
        "A bigger, blacker dick.",
        "Upgrading homeless people to mobile hotspots.",
        "Mild autism.",
        "The mere concept of Applebee's.",
        "Wearing an octopus for a hat.",
        "Nunchuck moves.",
        "A sad fat dragon with no friends.",
        "An unhinged ferris wheel rolling toward the sea.",
        "Whipping a disobedient slave.",
        "Catastrophic urethral trauma.",
        "Living in a trashcan.",
        "An ether-soaked rag.",
        "Hillary Clinton's death stare.",
        "The corporations.",
        "A sweet spaceship.",
        "Existing.",
        "A magic hippie love cloud.",
        "A 55-gallon drum of lube.",
        "A piñata full of scorpions.",
        "Fuck Mountain.",
        "Special musical guest, Cher.",
        "Mooing.",
        "Survivor's guilt.",
        "The human body.",
        "Swiftly achieving orgasm.",
        "Getting hilariously gang-banged by the Blue Man Group.",
        "Boris the Soviet Love Hammer.",
        "Daddy's belt.",
        "Jeff Goldblum.",
        "The grey nutrient broth that sustains Mitt Romney.",
        "Me.",
        "Making a friend.",
        "Tiny nipples.",
        "Double penetration.",
        "A soulful rendition of \"Ol' Man River.\"",
        "Oncoming traffic.",
        "Weapons-grade plutonium.",
        "Intimacy problems.",
        "A dollop of sour cream.",
        "Some really fucked up shit.",
        "A sweaty, panting leather daddy.",
        "A slightly shittier parallel universe.",
        "Subduing a grizzly bear and making her your wife.",
        "Spring break!",
        "My first kill.",
        "Rising from the grave.",
        "Being awesome at sex.",
        "Graphic violence, adult language, and some sexual content.",
        "The mixing of the races.",
        "Dining with cardboard cutouts of the cast of \"Friends.\"",
        "Fetal alcohol syndrome.",
        "Taking a man's eyes and balls out and putting his eyes where his balls go and then his balls in the eye holes.",
        "Another shot of morphine.",
        "The day the birds attacked.",
        "Scrotal frostbite.",
        "Beefin' over turf.",
        "One Ring to rule them all.",
        "All of this blood.",
        "A squadron of moles wearing aviator goggles.",
        "Grandpa's ashes.",
        "Loki, the trickster god.",
        "Bullshit.",
        "Basic human decency.",
        "Whining like a little bitch.",
        "Pretty Pretty Princess Dress-Up Board Game®.",
        "A Burmese tiger pit.",
        "Pumping out a baby every nine months.",
        "The new Radiohead album.",
        "Death by Steven Seagal.",
        "Tongue.",
        "An army of skeletons.",
        "The Google.",
        "Finding Waldo.",
        "A man in yoga pants with a ponytail and feather earrings.",
        "Power.",
        "Gift-wrapping a live hamster.",
        "Elf cum.",
        "A Hungry-Man Frozen Christmas Dinner for One.",
        "Several intertwining love stories featuring Hugh Grant.",
        "Mall Santa.",
        "Whatever Kwanzaa is supposed to be about.",
        "Clearing a bloody path through Walmart with a scimitar.",
        "Santa's heavy sack.",
        "Krampus, the Austrian Christmas monster.",
        "Another shitty year.",
        "My hot cousin.",
        "A Christmas stocking full of coleslaw.",
        "A visually arresting turtleneck.",
        "Fucking up \"Silent Night\" in front of 300 parents.",
        "Space Jam on VHS.",
        "A toxic family environment.",
        "Taking down Santa with a surface-to-air missile.",
        "Immaculate conception.",
        "Socks.",
        "The tiny, calloused hands of the Chinese children that made this card.",
        "The Star Wars Holiday Special.",
        "Pretending to be happy.",
        "Eating an entire snowman.",
        "Flying sex snakes",
        "Being a classy motherfucker",
        "The Little Town of Deathlaham",
        "A freak pancake accident",
        "Burning incriminating evidence",
        "Emma Watson",
        "For Science!",
        "Forgetting to delete your internet history",
        "That's what she said",
        "Mystery Science Theater 3000(tm)",
        "When your thong rides up",
        "Man caves",
        "A family reunion gone wrong",
        "A rogue journalist outside of the law",
        "Thunder Thighs",
        "A big black dick",
        "A bigger blacker dick",
        "Finding Waldo",
        "Hellen Keller's sex life",
        "Walking in on your parents having sex",
        "A murderous unicorn",
        "Reddit",
        "Mecha-Hitler",
        "Acting black",
        "Projectile vomiting",
        "Living with a hot thirteen year old",
        "Genocide",
        "Coat hanger abortions",
        "Man meat",
        "Auto-cannibalism",
        "Vigorous jazz hands",
        "Flightless birds",
        "Pictures of boobs",
        "Doing the right thing",
        "Hunting accidents",
        "A cartoon camel enjoying the smooth, refreshing taste of a cigarette",
        "The violation of our most basic human rights",
        "Viagra (r)",
        "Self-loathing",
        "Spectacular abs",
        "An honest cop with nothing left to lose",
        "Abstinence",
        "A balanced breakfast",
        "Mountain Dew Code Red",
        "Concealing a boner",
        "Roofies",
        "Glenn Beck convulsively vomiting as a brood of crab spiders hatches in his brain and erupts from his tear ducts",
        "Tweeting",
        "The Big Bang",
        "Amputees",
        "Dr. Martin Luther King, Jr.",
        "Former President George W. Bush",
        "Being marginalized",
        "Smegma",
        "Laying an egg",
        "Cuddling",
        "Aaron Burr",
        "The Pope",
        "A bleached asshole",
        "Horse meat",
        "Genital piercings",
        "Fingering",
        "Elderly Japanese men",
        "Stranger danger",
        "Fear itself",
        "Science",
        "Praying the gay away",
        "Same-sex ice dancing",
        "The terrorists",
        "Making sex at here",
        "German dungeon porn",
        "Bingeing and purging",
        "Ethnic cleansing",
        "Cheating in the Special Olympics",
        "Nickelback",
        "Heteronormativity",
        "William Shatner",
        "Making a pouty face",
        "Chainsaws for hands",
        "The placenta",
        "The profoundly handicapped",
        "Tom Cruise",
        "Object permanence",
        "Goblins",
        "An icepick lobotomy",
        "Arnold Schwarzenegger",
        "Hormone injections",
        "A falcon with a cap on its head",
        "Foreskin",
        "Dying",
        "Stunt doubles",
        "The invisible hand",
        "Jew-fros",
        "A really cool hat",
        "Flash flooding",
        "Flavored condoms",
        "Dying of dysyntery",
        "Sexy pillow fights",
        "The Three-Fifths compromise",
        "A sad handjob",
        "Men",
        "Historically black colleges",
        "Sean Penn",
        "Heartwarming orphans",
        "Waterboarding",
        "The clitoris",
        "Vikings",
        "Friends who eat all the snacks",
        "The Underground Railroad",
        "Pretending to care",
        "Raptor attacks",
        "A micropenis",
        "A Gypsy curse",
        "Agriculture",
        "Bling",
        "A clandestine butt scratch",
        "The South",
        "Sniffing glue",
        "Consultants",
        "My humps",
        "Geese",
        "Being a dick to children",
        "Party poopers",
        "Sunshine and rainbows",
        "YOU MUST CONSTRUCT ADDITIONAL PYLONS",
        "Mutually-assured destruction",
        "Heath Ledger",
        "Sexting",
        "An Oedipus complex",
        "Eating all of the cookies before the AIDS bake-sale",
        "A sausage festival",
        "Michael Jackson",
        "Skeletor",
        "Chivalry",
        "Sharing needles",
        "Being rich",
        "Muzzy",
        "Count Chocula",
        "Spontaneous human combustion",
        "College",
        "Necrophilia",
        "The Chinese gymnastics team",
        "Global warming",
        "Farting and walking away",
        "Emotions",
        "Uppercuts",
        "Cookie Monster devouring the Eucharist wafers",
        "Stifling a iggle at the mention of Hutus and Tutsis",
        "Penis envy",
        "Letting yourself go",
        "White people",
        "Dick Cheney",
        "Leaving an awkward voicemail",
        "Yeast",
        "Natural selection",
        "Masturbation",
        "Twinkies (r)",
        "A LAN Party",
        "Opposable thumbs",
        "A grande sugar-free iced soy caramel macchiato",
        "Soiling oneself",
        "A sassy black woman",
        "Sperm whales",
        "Teaching a robot to love",
        "Scrubbing under the folds",
        "A drive-by shooting",
        "Whipping it out",
        "Panda sex",
        "Catapults",
        "Will Smith",
        "Toni Morrison's vagina",
        "Five-Dollar Foot-longs (tm)",
        "Land minds",
        "A sea of troubles",
        "A zesty breakfast burrito",
        "Christopher Walken",
        "Friction",
        "Balls",
        "AIDS",
        "The KKK",
        "Figgy pudding",
        "Seppuku",
        "Marky Mark and the Funky Bunch",
        "Gandhi",
        "Dave Matthews Band",
        "Preteens",
        "The token minority",
        "Friends with benefits",
        "Re-gifting",
        "Pixelated bukkake",
        "Substitute teachers",
        "Take-backsies",
        "A thermonuclear detonation",
        "The Tempur-Pedic (r) Swedish Sleep System (tm)",
        "Waiting 'til marriage",
        "A tiny horse",
        "A can of whoop-ass",
        "Dental dams",
        "Feeding Rosie O'Donnell",
        "Old-people smell",
        "Genghis Khan",
        "Authentic Mexican cuisine",
        "Oversized lollipops",
        "Garth Brooks",
        "Keanu Reeves",
        "Drinking alone",
        "The American Dream",
        "Taking off your shirt",
        "Giving 110%",
        "Flesh-eating bacteria",
        "Child abuse",
        "A cooler full of organs",
        "A moment of silence",
        "The Rapture",
        "Keeping Christ in Christmas",
        "RoboCop",
        "That one gay Teletubby",
        "Sweet, sweet vengeance",
        "Fancy Feast (r)",
        "Pooping back and forth. Forever.",
        "Being a motherfucking sorcerer",
        "Jewish fraternities",
        "Edible underpants",
        "Poor people",
        "All-you-can-eat shrimp for $4.99",
        "Britney Spears at 55",
        "That thing that electrocutes your abs",
        "The folly of man",
        "Fiery poops",
        "Cards Against Humanity",
        "A murder most foul",
        "Me time",
        "The inevitable heat death of the universe",
        "Nocturnal emissions",
        "Daddy issues",
        "The hardworking Mexican",
        "Natalie Portman",
        "Waking up half-naked in a Denny's parking lot",
        "Nipple blades",
        "Assless chaps",
        "Full frontal nudity",
        "Hulk Hogan",
        "Passive-aggression",
        "Ronald Reagan",
        "Vehicular manslaughter",
        "Menstruation",
        "Pulling out",
        "Picking up girls at the abortion clinc",
        "The homosexual agenda",
        "The Holy Bible",
        "World peace",
        "Dropping a chandelier on your enemies and riding the rope up",
        "Testicular torsion",
        "The milk man",
        "A time-travel paradox",
        "Hot Pockets (r)",
        "Guys who don't call",
        "Eating the last known bison",
        "Darth Vader",
        "Scalping",
        "Homeless people",
        "The World of Warcraft",
        "Gloryholes",
        "Saxophone solos",
        "Sean Connery",
        "God",
        "Intelligent design",
        "The taint; the grundle; the fleshy fun-bridge",
        "Friendly fire",
        "Keg stands",
        "Eugenics",
        "A good sniff",
        "Lockjaw",
        "A neglected Tamagotchi (tm)",
        "The People's Elbow",
        "Robert Downey, Jr.",
        "The heart of a child",
        "Seduction",
        "Smallpox blankets",
        "Licking things to claim them as your own",
        "A salty surprise",
        "Poorly-timed Holocaust jokes",
        "My soul",
        "My sex life",
        "Pterodactyl eggs",
        "Altar boys",
        "Forgetting the Alamo",
        "72 virgins",
        "Raping and pillaging",
        "Pedophiles",
        "Eastern European Turbo-folk music",
        "A snapping turtle biting the tip of your penis",
        "Pabst Blue Ribbon",
        "Domino's (tm) Oreo (tm) Dessert Pizza",
        "My collection of high-tech sex toys",
        "A middle-aged man on roller skates",
        "The Blood of Christ",
        "Half-assed foreplay",
        "Free samples",
        "Douchebags on their iPhones",
        "Hurricane Katrina",
        "Wearing underwear inside-out to avoid doing laundry",
        "Republicans",
        "The glass ceiling",
        "A foul mouth",
        "Jerking off into a pool of children's tears",
        "Getting really high",
        "The deformed",
        "Michelle Obama's arms",
        "Explosions",
        "The Übermensch",
        "Donald Trump",
        "Sarah Palin",
        "Attitude",
        "This answer is postmodern",
        "Crumpets with the Queen",
        "Frolicking",
        "Team-building exercises",
        "Repression",
        "Road head",
        "A bag of magic beans",
        "An asymmetric boob job",
        "Dead parents",
        "Public ridicule",
        "A mating display",
        "A mime having a stroke",
        "Stephen Hawking talking dirty",
        "African children",
        "Mouth herpes",
        "Overcompensation",
        "Bill Nye the Science Guy",
        "Bitches",
        "Italians",
        "Have some more kugel",
        "A windmill full of corpses",
        "Her Royal Highness, Queen Elizabeth II",
        "Crippling debt",
        "Adderall (tm)",
        "A stray pube",
        "Shorties and blunts",
        "Passing a kidney stone",
        "Prancing",
        "Leprosy",
        "A brain tumor",
        "Bees?",
        "Puppies!",
        "Cockfights",
        "Kim Jong-Il",
        "Hope",
        "8 oz. of sweet Mexican black-tar heroin",
        "Incest",
        "Grave robbing",
        "Asians who aren't good at math",
        "Alcoholism",
        "(I am doing Kegels right now.)",
        "Justin Bieber",
        "The Jews",
        "Bestiality",
        "Winking at old people",
        "Drum circles",
        "Kids with ass cancer",
        "Loose lips",
        "Auschwitz",
        "Civilian casualties",
        "Inappropriate yodeling",
        "Tangled Slinkys",
        "Being on fire",
        "The Thong Song",
        "A vajazzled vagina",
        "Riding off into the sunset",
        "Exchanging pleasantries",
        "My relationship status",
        "Shaquille O'Neals's acting career",
        "Being fabulous",
        "Lactation",
        "Not reciprocating oral sex",
        "Sobbing into a Hungry-Man (r) Frozen Dinner",
        "My genitals",
        "Date rape",
        "Ring Pops (tm)",
        "GoGurt",
        "Judge Judy",
        "Lumberjack fantasies",
        "The gays",
        "Scientology",
        "Estrogen",
        "Police brutality",
        "Passable transvestites",
        "The Virginia Tech Massacre",
        "Tiger Woods",
        "Dick fingers",
        "Racism",
        "Glenn Beck being harried by a swarm of buzzards",
        "Surprise sex!",
        "Classist undertones",
        "Booby-trapping the house to foil burglars",
        "New Age music",
        "PCP",
        "A lifetime of sadness",
        "Doin' it in the butt",
        "Swooping",
        "The Hamburglar",
        "Tentacle porn",
        "A hot mess",
        "Too much hair gel",
        "A look-see",
        "Not giving a shit about the Third World",
        "American Gladiators",
        "The Kool-Aid man",
        "Mr Snuffleupagus",
        "Barack Obama",
        "Golden showers",
        "Wiping her butt",
        "Queefing",
        "Getting drunk on mouthwash",
        "An M. Night Shyamalan plot twist",
        "A robust mongoloid",
        "Nazis",
        "White privilege",
        "An erection that lasts longer than four hours",
        "A disappointing birthday party",
        "Puberty",
        "Two midgets shitting in a bucket",
        "Wifely duties",
        "The forbidden fruit",
        "Getting so angry that you pop a boner",
        "Sexual tension",
        "Third base",
        "A gassy antelope",
        "Those times when you get sand in your vagina",
        "A Super Soaker (tm) full of cat pee",
        "Muhammad (Praise Be Unto Him)",
        "Racially-biased SAT questions",
        "Porn stars",
        "A fetus",
        "Obesity",
        "When you fart and a little bit comes out",
        "Oompa-Loompas",
        "BATMAN!!!",
        "Black people",
        "Tasteful sideboob",
        "Hot people",
        "Grandma",
        "Copping a feel",
        "The Trail of Tears",
        "Famine",
        "Finger painting",
        "The miracle of childbirth",
        "Goats eating cans",
        "A monkey smoking a cigar",
        "Faith healing",
        "Parting the Red Sea",
        "Dead babies",
        "The Amish",
        "Impotence",
        "Child beauty pageants",
        "Centaurs",
        "AXE Body Spray",
        "Kanye West",
        "Women's suffrage",
        "Children on leashes",
        "Harry Potter erotica",
        "The Dance of the Sugar Plum Fairy",
        "Lance Armstrong's missing testicle",
        "Dwarf tossing",
        "Mathletes",
        "Lunchables (tm)",
        "Women in yogurt commercials",
        "John Wilkes Booth",
        "Powerful thighs",
        "Mr. Clean, right behind you",
        "Multiple stab wounds",
        "Cybernetic enhancements",
        "Serfdom",
        "Another god-damn vampire movie",
        "Glenn Beck catching his scrotum on a curtain hook",
        "A big hoopla about nothing",
        "Peeing a little bit",
        "The Hustle",
        "Ghosts",
        "Bananas in Pajamas",
        "Active listening",
        "Dry heaving",
        "Kamikaze pilots",
        "The Force",
        "Anal beads",
        "The Make-A-Wish (r) Foundation"
    ]
}
